...
const UpdateBusScheduleForm = ({ onSubmit, onClose, auth, existingSchedule }) => {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            bus: existingSchedule.bus?._id || "",
            availableSeats: existingSchedule.availableSeats || "",
            route: existingSchedule.route?._id || "",
            ticketPrice: existingSchedule.ticket_price?.$numberDecimal || "",
        },
    ...
    // Set bus capacity when a bus is selected
    useEffect(() => {
        if (existingSchedule && buses.length > 0) {
            const selectedBus = buses.find((bus) => bus.value === existingSchedule.bus?._id);
            setBusCapacity(selectedBus?.capacity || null);
        }
    }, [existingSchedule, buses]);
    ...
    return (
        <form onSubmit={formik.handleSubmit} className="space-y-4">
            ...
            <div>
                ...
                <Select
                    options={buses}
                    value={buses.find((bus) => bus.value === formik.values.bus)}
                    onChange={(option) => {
            ...
            <div>
                ...
                <Select
                    options={routes}
                    value={routes.find((route) => route.value === formik.values.route)}
...
export default UpdateBusScheduleForm;
